...
public class RepaymentPlanB {
    ...
    public RepaymentPlanB(BigDecimal principal, BigDecimal annualRate, int periods) {
    ...
    private void calculate() {
        ...
        // 计算每月还款金额
        monthlyPayment = principal.multiply(monthlyRate).multiply(monthlyRate.add(BigDecimal.ONE).pow(periods))
                .divide(monthlyRate.add(BigDecimal.ONE).pow(periods).subtract(BigDecimal.ONE), 2, BigDecimal.ROUND_HALF_UP);
        // 计算每期的还款情况
        for (int i = 1; i <= periods; i++) {
            Repayment repayment = new Repayment();
            repayment.setPeriod(i); // 设置期数
            // 计算剩余本金
            BigDecimal remainingPrincipal = principal.subtract(monthlyPayment.subtract(principal.multiply(monthlyRate))
                    .multiply(new BigDecimal(i - 1))).setScale(2, BigDecimal.ROUND_HALF_UP);
            ...
            // 计算每月还款本金
            BigDecimal monthlyPrincipal = monthlyPayment.subtract(monthlyInterest).setScale(2, BigDecimal.ROUND_HALF_UP);
